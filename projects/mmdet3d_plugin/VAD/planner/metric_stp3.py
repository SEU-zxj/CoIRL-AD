...
class PlanningMetric():
    ...
    def evaluate_coll_batch(
        ...
        segmentation: torch.Tensor (B, n_future, 200, 200)
        Return:
        obj_coll_sum (Tensor): [B, T]
        obj_box_coll_sum (Tensor): [B, T]
        '''
        B, n_future, _ = trajs.shape
        # trajs = trajs * torch.tensor([-1, 1], device=trajs.device)
        # gt_trajs = gt_trajs * torch.tensor([-1, 1], device=gt_trajs.device)

        obj_coll_sum = torch.zeros((B, n_future), device=segmentation.device)
        obj_box_coll_sum = torch.zeros((B, n_future), device=segmentation.device)
        ...
        for i in range(B):
            ...
            ti = torch.arange(n_future)
            obj_coll_sum[i, ti[m1]] += segmentation[i, ti[m1], xi[m1], yi[m1]].long()

            m2 = torch.logical_not(gt_box_coll)
            box_coll = self.evaluate_single_coll(trajs[i], segmentation[i], input_gt=False).to(segmentation.device)
            obj_box_coll_sum[i, ti[m2]] += (box_coll[ti[m2]]).long()
