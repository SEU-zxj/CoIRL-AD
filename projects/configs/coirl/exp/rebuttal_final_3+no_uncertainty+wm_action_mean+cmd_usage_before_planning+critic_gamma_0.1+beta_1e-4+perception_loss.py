...
model = dict(
    ...
    pts_bbox_head=dict(
        type='VADHead_CoIRL',
        perception_mode='perception-based', # 'perception-free' or 'perception-based' (add auxiliary tasks: detection+map reconstruction+agent motion prediction)
        map_thresh=0.5,
        dis_thresh=0.2,
        ...
        valid_fut_ts=6,
        #===============ego_agent_decoder and ego_map_decoder are serve for planning, we do not need then here
        ...
        transformer=dict(
            type='VADPerceptionTransformer_CoIRL',
            map_num_vec=map_num_vec,
            map_num_pts_per_vec=map_fixed_ptsnum_per_pred_line,
...
dataset_type = 'VADCustomNuScenesDataset_CoIRL'
